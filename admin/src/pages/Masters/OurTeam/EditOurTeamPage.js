...
import { useUpdateOurTeamMaster, useViewOurTeamMaster } from 'hooks';
import { imagePreviewFromik, defaultValue, fileToDataUri, s3BucketFileUpload } from 'helpers';
import { TNBreadCurm } from 'common/components';

const EditOurTeamPage = ({ t }) => {
  const navigate = useNavigate();
  const memberImageRef = useRef();

  const { our_team_id } = useParams();

  /**
   * !This API will call when user click on Submit Button, and user will be redirecting to the listing page
   */
  const { mutate: doUpdateOurTeamMaster, isLoading } = useUpdateOurTeamMaster((response) => {
   ...
   */
  useViewOurTeamMaster(our_team_id, ({ data: our_team }) => {
    if (our_team) {
      formik.values.name = our_team.name;
      formik.values.designation = our_team.designation;
      formik.values.is_active = our_team.is_active.toString();
      formik.values.our_team_image = our_team.image;
    }
  });
  ...
  const formik = useFormik({
    ...
    onSubmit: async (values) => {
      values.our_team_id = our_team_id;
      doUpdateOurTeamMaster(values);
    },
  });
  /**
  ...
  const breadcurmArray = [
    ...
    {
      label: t('page.edit_our_team_label'),
      link: '',
      active: 'active',
    },
  ];
  ...
  return (
    <>
      ...
      <Card className="inner-box">
        <h1 className="page-heading-center">{t('page.edit_our_team_label')}</h1>
