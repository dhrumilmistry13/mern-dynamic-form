...
import { useGetInsuranceDetails } from 'hooks';
...
const ViewInsuranceDetails = ({ t }) => {
  ...
  // Initial Values for Insurance Questions and Ans
   ...
   */
  const { isLoading: isLoadingBasciData, data: insuranceData } = useGetInsuranceDetails({
    user_id,
  });
  if (!isLoadingBasciData && insuranceData) {
    insuranceDetails =
      insuranceData.data?.length >= 0 &&
      insuranceData.data?.map((data) => {
        let document_type;
  ...
  const breadcurmArray = [
    ...
    {
      label: t('page.view_insurance_detail_patient_label'),
      link: '',
      active: 'active',
    },
  ];
      ...
      <Card className="inner-box">
        <Card.Body>
          <h1 className="page-heading-center ">
            {t('page.patient_insurance_questions_and_ans_label')}
          </h1>
          {!isLoadingBasciData ? (
            insuranceData && (
              <Row>
                {insuranceDetails?.length > 0 ? (
                  insuranceDetails.map((queAndAns, i) => (
                    <Col lg={12} key={i}>
                      <Row>
                        <Col lg={12} md={12} sm={12}>
                          <Row>
                            <Col lg={1} xs={6}>
                              <Form.Label className="field-label">
                                {t('page.patient_insurance_question_label')} - {i + 1}
                              </Form.Label>
                            </Col>
                          ...
                          {[3, 4, 5].includes(
                            ...
                            queAndAns.option_value && queAndAns.option_value.length > 0 ? (
                              <Row>
                                <Col lg={1} xs={6}>
                                  <Form.Label className="field-label">
                                    {t('page.patient_insurance_question_option_label')}
                                  </Form.Label>
                                </Col>
                                <Col lg={1} xs={1} className="divider">
                                  :
                            ...
                            <Row>
                              <Col lg={1} xs={6}>
                                <Form.Label className="field-label">
                                  {t('page.patient_insurance_question_answer_label')}
                                </Form.Label>
                              </Col>
                              <Col lg={1} xs={1} className="divider">
                                :
                              ...
                              {queAndAns.document_type === 1 ? (
                                ...
                                <Col lg={9} xs={5} className="my-3">
                                  <a
                                    ...
                                    {t('page.patient_insurance_question_response_download_file')}
                                  </a>
                                </Col>
                              )}
                            </Row>
                  ...
                  <h5 className="page-heading-center">
                    {t('page.patient_not_added_insurance_text')}
                  </h5>
                )}
              </Row>
            )
          ) : (
            <h5 className="page-heading-center">{t('page.patient_loading_data_text')}</h5>
          )}
...
ViewInsuranceDetails.propTypes = {
  t: PropTypes.func,
};
