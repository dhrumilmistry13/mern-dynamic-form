...
import { useAddTranslation } from 'hooks';

const AddTranslationPage = ({ t }) => {
  const navigate = useNavigate();

  /**
   * !This API will call when user click on Submit Button, and user will be redirected to the listing page
   */
  const { mutate: doAddTranslation, isLoading } = useAddTranslation((response) => {
  ...
  const breadcurmArray = [
    ...
    {
      label: t('page.add_translation_label'),
      link: '',
      active: 'active',
    },
  ];
  ...
  return (
    <>
      ...
      <Card className="inner-box">
        <h1 className="page-heading-center">{t('page.add_translation_label')}</h1>
