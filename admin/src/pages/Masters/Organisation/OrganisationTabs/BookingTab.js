...
const BookingListOrganisationPage = ({ t }) => {
  ...
  const { data: timezone } = useGetOrgTimezoneData(user_id);
  ...
  //  */
  const options = [
    { value: 1, label: `${t('page.organization_booking_tab_pending_text')}` },
    { value: 2, label: `${t('page.organization_booking_tab_accepted_text')}` },
    { value: 3, label: `${t('page.organization_booking_tab_rejected_text')}` },
    { value: 4, label: `${t('page.organization_booking_tab_completed_text')}` },
    { value: 5, label: `${t('page.organization_booking_tab_no_show_both_text')}` },
    { value: 6, label: `${t('page.organization_booking_tab_no_show_patient_text')}` },
    { value: 7, label: `${t('page.organization_booking_tab_no_show_organization_text')}` },
    { value: 8, label: `${t('page.organization_booking_tab_reschedule_text')}` },
    { value: 9, label: `${t('page.organization_booking_tab_cancel_by_system_text')}` },
    { value: 10, label: `${t('page.organization_booking_tab_status_Cancel_text')}` },
  ];
  const dateFromRef = useRef();
  const dateToRef = useRef();

  /**
   * !This API will call when Page Render then get Patient List for Options
   */
  const { isLoading: patientListIsLoad, data: patientList } = useGetPatientData(user_id);
  const optionsPatient = [];
  if (patientList !== undefined) {
    patientList.data.map((val) => {
      return optionsPatient.push({
        value: val.user_id,
  ...
  /**
   * !This API will call When Page Render then get Timezone List for Options
   */
  ...
  /**
   * This function will set Search Text
   */
  ...
  /**
   * This function will set Booking Data And Show Notes Model
   */
  ...
  const columnsjson = [
    {
      Header: `${t('page.organisation_booking_tab_created_at_label')}`,
      accessor: 'created_at',
      disableSortBy: true,
      Cell: ({ row }) => {
    ...
    {
      Header: `${t('page.organisation_booking_tab_full_name_label')}`,
      accessor: 'first_name',
      disableSortBy: true,
      Cell: ({ row }) => {
        return (
    ...
    {
      Header: `${t('page.organisation_booking_tab_booking_id_label')}`,
    ...
    {
      Header: `${t('page.organisation_tab_booking_book_date_label')}`,
      accessor: 'book_date',
      disableSortBy: true,
      Cell: ({ row }) => {
        return (
    ...
    {
      Header: `${t('page.organisation_booking_tab_start_time_label')}`,
      accessor: 'start_time',
      disableSortBy: true,
      Cell: ({ row }) => {
    ...
    {
      Header: `${t('page.organisation_booking_tab_booking_status_label')}`,
      accessor: 'user_id',
      disableSortBy: true,
      Cell: ({ row }) => {
        if (row.original.booking_status === 1)
          return (
            <Badge className="warning">{t('page.organization_booking_tab_pending_text')}</Badge>
          );
        else if (row.original.booking_status === 2)
          return (
            <Badge className="primary">{t('page.organization_booking_tab_accepted_text')}</Badge>
          );
        else if (row.original.booking_status === 3)
          return (
            <Badge className="rejected">{t('page.organization_booking_tab_rejected_text')}</Badge>
          );
        else if (row.original.booking_status === 4)
          return (
            <Badge className="completed">{t('page.organization_booking_tab_completed_text')}</Badge>
          );
        else if (row.original.booking_status === 5)
          return (
            <Badge className="no-show-both">
              {t('page.organization_booking_tab_no_show_both_text')}
            </Badge>
          );
        else if (row.original.booking_status === 6)
          return (
            <Badge className="no-show-patient">
              {t('page.organization_booking_tab_no_show_patient_text')}
            </Badge>
          );
        else if (row.original.booking_status === 7)
          return (
            <Badge className="no-show-org">
              {t('page.organization_booking_tab_no_show_organization_text')}
            </Badge>
          );
        else if (row.original.booking_status === 8)
          return (
            <Badge className="rescheduale-org">
              {t('page.organization_booking_tab_reschedule_organization_text')}
            </Badge>
          );
        else if (row.original.booking_status === 9)
          return (
            <Badge className="cancel-by-sys">
              {t('page.organization_booking_tab_cancel_by_system_text')}
            </Badge>
          );
        else if (row.original.booking_status === 10)
          return (
            <Badge className="cancel-status">
              {t('page.organization_booking_tab_cancel_status_text')}
            </Badge>
          );
      },
    },
    {
      Header: `${t('page.organisation_booking_tab_booking_action_label')}`,
              ...
              <>
                <div className="action_btn">
                  <TNButton
                    ...
                    {t('page.booking_list_action_button_view_text')}
                  </TNButton>
                </div>
                <Badge className="cancel-status">
                  {t('page.booking_tab_booking_canceled_by_patient_text')}
                </Badge>
              </>
            );
          }
          if (row.original.cancellation_by === 2) {
            return (
              <>
                ...
                <Badge className="cancel-status">
                  {t('page.booking_tab_canceled_by_organization_text')}
                </Badge>
              </>
            );
          }
          if (row.original.cancellation_by === 3) {
            return (
              <>
                ...
                <Badge className="cancel-status">
                  {t('page.booking_tab_canceled_by_admin_text')}
                </Badge>
              </>
            );
          }
   ...
   */
  const { refetch } = useOrganizationBookingList(
    {
      page: currentPage,
      user_id,
  ...
  /**
   * This function will Redirect to Organization List Page When Call
   */
  const handleCancel = () => {
  ...
  const breadcurmArray = [
    ...
    {
      label: t('page.view_booking_list_organisation_label'),
      link: '',
      active: 'active',
    },
  ];
              ...
              <div className="filter">
                <Form.Group className="filter-field-space">
                  <Select
                    ...
                    isMulti
                    options={options}
                    placeholder={t('page.organisation_booking_tab_filter_status_label')}
              ...
              <div className="filter">
                <Form.Group className="filter-field-space">
                  <Select
                    ...
                    placeholder={t('page.organization_booking_tab_timezone_label')}
                ...
                <Form.Group className="filter-field-space">
                  <Select
                    ...
                    isMulti={true}
                    options={patientListIsLoad ? optionsPatient : optionsPatient}
                    placeholder={t('page.organisation_booking_tab_patient_filter_label')}
                    value={defaultValue(optionsPatient, selectedPatientIds.split(','))}
                    onChange={(selectedOption) => {
                      let patientArr = [];
                      selectedOption &&
                        selectedOption?.map((op) => {
                          patientArr.push(op.value);
                          return patientArr;
                        });
                      let strVal = patientArr.join(',');
                      setSelectedPatientIds(strVal);
                      setCurrentPage(1);
                    }}
                  />
                </Form.Group>
